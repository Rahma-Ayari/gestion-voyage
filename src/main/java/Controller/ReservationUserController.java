...
public class ReservationUserController implements Initializable {
    ...
    private void loadOffres() {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur chargement offres : " + e.getMessage());
        }
    }
    ...
    private VBox creerCarte(Offre offre) {
        ...
        btnRes.setOnAction(e -> naviguerVersFormulaire(offre));
    ...
    // ══════════════════════════════════════════════════
    //  NAVIGATION → PAGE RÉSERVATION
    // ══════════════════════════════════════════════════

    private void naviguerVersFormulaire(Offre offre) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/ReservationUser/ReservationForm.fxml"));
            Parent root = loader.load();

            ReservationFormController ctrl = loader.getController();
            ctrl.initOffre(offre);

            Stage stage = (Stage) offresGrid.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.show();

        } catch (Exception e) {
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Impossible d'ouvrir la page de réservation : " + e.getMessage());
        }
    }

    // ══════════════════════════════════════════════════
    //  NAVIGATION → PAGE CONFIGURATION VOYAGE (NOUVEAU)
    // ══════════════════════════════════════════════════

    @FXML
    private void ouvrirConfigurationVoyage() {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/ConfigurerVoyage/ConfigVoyageUser.fxml")  // Adaptez le chemin selon votre structure
            );
            Parent root = loader.load();

            Stage stage = (Stage) configurerVoyageBtn.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.setTitle("Configurer un nouveau voyage");
            stage.show();

        } catch (IOException e) {
            showAlert(Alert.AlertType.ERROR,
                    "Impossible d'ouvrir la page de configuration : " + e.getMessage());
            e.printStackTrace();
        }
    }

    // ══════════════════════════════════════════════════
    //  POPUP DÉTAIL COMPLET (bouton "Voir plus")
    // ══════════════════════════════════════════════════
    ...
    private void ouvrirDetail(Offre offre) {
        ...
        headerImg.setFitWidth(620); headerImg.setFitHeight(220); headerImg.setPreserveRatio(false);
        ...
        typeBadge.setStyle("-fx-background-color: #FF6B35; -fx-text-fill: white;" +
                "-fx-font-size: 10px; -fx-font-weight: bold; -fx-background-radius: 4; -fx-padding: 3 10;");
        ...
        descCourt.setWrapText(true); descCourt.setMaxWidth(560);
        ...
        if (offre.getDateDebut() != null || offre.getDateFin() != null) {
            ...
            if (offre.getDateDebut() != null && offre.getDateFin() != null) {
                ...
                dateCards.getChildren().add(carteInfo("⏱ Durée", j + " jours", "#E3F2FD", "#1565C0"));
            }
        ...
        if (offre.getDestination() != null) {
            ...
            card.getChildren().addAll(
                    ligneDetail("🗺 Pays", d.getPays() != null ? d.getPays() : "—"), separateur(),
                    ligneDetail("🏙 Ville", d.getVille() != null ? d.getVille() : "—"), separateur(),
                    ligneDetail("📝 Description", d.getDescription() != null ? d.getDescription() : "—")
            );
            corps.getChildren().add(card);
        }
        ...
        if (offre.getVol() != null) {
            ...
            VBox card = sectionCard();
            card.getChildren().addAll(
                    ligneDetail("🔢 Numéro", v.getNumeroVol() != null ? v.getNumeroVol() : "—"), separateur(),
                    ligneDetail("🏢 Compagnie", v.getCompagnie() != null ? v.getCompagnie() : "—"), separateur(),
                    ligneDetail("🛫 Départ", v.getDateDepart() != null ? v.getDateDepart().format(FMT_DT) : "—"), separateur(),
                    ligneDetail("🛬 Arrivée", v.getDateArrivee() != null ? v.getDateArrivee().format(FMT_DT) : "—"), separateur(),
                    ligneDetail("💰 Prix vol", String.format("%.0f €", v.getPrix()))
            );
            if (v.getDestination() != null)
                card.getChildren().addAll(separateur(),
                        ligneDetail("📍 Vers", v.getDestination().getVille() + ", " + v.getDestination().getPays()));
            corps.getChildren().add(card);
        }
        ...
        if (offre.getHotel() != null) {
            ...
            VBox card = sectionCard();
            card.getChildren().addAll(
                    ligneDetail("🏷 Nom", h.getNom() != null ? h.getNom() : "—"), separateur(),
                    ligneDetail("⭐ Étoiles", etoiles), separateur(),
                    ligneDetail("🏙 Ville", h.getVille() != null ? h.getVille() : "—"), separateur(),
                    ligneDetail("📍 Adresse", h.getAdresse() != null ? h.getAdresse() : "—"), separateur(),
                    ligneDetail("🛏 Chambre", h.getTypeChambre() != null ? h.getTypeChambre() : "—"), separateur(),
        ...
        if (offre.getActivite() != null) {
            ...
            VBox card = sectionCard();
            card.getChildren().addAll(
                    ligneDetail("🏷 Nom", a.getNom() != null ? a.getNom() : "—"), separateur(),
                    ligneDetail("📂 Catégorie", a.getCategorie() != null ? a.getCategorie() : "—"), separateur(),
                    ligneDetail("⏱ Durée", a.getDureeEnHeure() + " heure(s)"), separateur(),
                    ligneDetail("🕐 Horaire", a.getHoraire() != null ? a.getHoraire() : "—"), separateur(),
        ...
        btnFermer.setStyle("-fx-background-color: #F0F0F0; -fx-text-fill: #555;" +
                "-fx-background-radius: 10; -fx-cursor: hand; -fx-font-size: 13px; -fx-padding: 10 24;");
        ...
        btnRes.setOnAction(e -> { dialog.close(); naviguerVersFormulaire(offre); });
        ...
        dialog.setScene(new Scene(root, 620, 700));
        dialog.showAndWait();
    }
    ...
    private HBox ligneDetail(String label, String valeur) {
        ...
        Label lbl = new Label(label); lbl.setMinWidth(140);
        lbl.setStyle("-fx-font-size: 12px; -fx-text-fill: #999; -fx-font-weight: bold;");
        Label val = new Label(valeur); val.setWrapText(true);
    ...
    private VBox carteInfo(String titre, String valeur, String bg, String color) {
        ...
        Label t = new Label(titre); t.setStyle("-fx-font-size: 10px; -fx-font-weight: bold; -fx-text-fill: " + color + ";");
        Label v = new Label(valeur); v.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: " + color + ";");
    ...
    private Separator separateur() {
        ...
        s.setStyle("-fx-background-color: #F5F5F5;");
        return s;
    }
